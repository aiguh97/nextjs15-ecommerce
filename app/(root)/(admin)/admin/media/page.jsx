...
const MediaPage = () => {
  ...
  return (
    <div>
      ...
      <Card className="py-0 rounded card-shadow">
        <CardHeader className="pt-3 pb-2 px-3 border-b [.border-b]:pb-2">
          <div className="flex justify-between items-center">
            <h4 className="font-semibold text-xl uppercase">
              {deleteType === "SD" ? "Media" : "Media Trash"}
            </h4>
            <div className="flex items-center gap-4">
              {deleteType === "SD" && <UploadMedia isMultiple />}

              <Link
                href={
                  deleteType === "SD"
                    ? `${ADMIN_MEDIA_SHOW}?trashof=media`
                    : ADMIN_MEDIA_SHOW
                }
              >
                <Button
                  type="button"
                  variant={deleteType === "SD" ? "destructive" : "outline"}
                  className="flex items-center gap-2"
                >
                  <LuTrash size={16} />
                  {deleteType === "SD" ? "Trash" : "Back to Media"}
                </Button>
              </Link>
            </div>
          </div>
