...
const LoginPage = () => {
  ...
  const handleLoginSubmit = async (values) => {
    try {
      ...
      if (data.data?.requireOtp) {
        // ✅ Hanya set otpEmail jika memang requireOtp true
        setOTPEmail(values.email);
      } else {
        // OTP sudah verified / tidak dibutuhkan
  ...
  const handleOTPVerification = async (values) => {
    try {
      ...
      // ✅ Hapus otpEmail agar tidak tampil lagi
      setOTPEmail("");
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleLoginSubmit)} className="space-y-5">
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input
                            ...
                            placeholder="••••••••"
                            className={`pr-10 ${fieldState.error ? "border-destructive focus-visible:ring-destructive" : ""}`}
                            {...field}
                          />
                          <button
                            type="button"
                ...
                <div className="flex justify-end">
                  <Link href={WEBSITE_RESET_PASSWORD} className="text-primary underline">
                    Forgot Password?
                  </Link>
                </div>
